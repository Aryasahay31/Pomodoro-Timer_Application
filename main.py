...
def start_timer():
    ...
    if reps % 8 == 0:
        ...
        timer_label.config(text="Break", fg= RED, font=(FONT_NAME, 50), bg = YELLOW)
    elif reps %2 == 0:
...
def count_down(count):
    ...
    canvas.itemconfig(timer_time, text = f"{count_min}:{count_sec}")
    if count > 0:
        global timer
...
canvas = Canvas(width=200, height=224, bg= YELLOW, highlightthickness=0) #same as the size of the image
